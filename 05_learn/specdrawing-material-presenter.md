...
Woodone「ラクラクプレゼン」(`/pboard/`) のアーキテクチャ調査（[[05_learn/woodone-pboard-architecture]]）を踏まえ、**ベースパース画像 + マスク合成で任意色対応**する自作プレゼンボードの MVP を OpenSpec ワークフローで仕様化〜実装〜GitHub push まで一気通貫で構築。リポジトリ: <https://github.com/tatoflam/SpecDrawing> (`b9b297b`、77 files / 12,387 行)。MVP 後も `add-base-variant-finishes` → `add-multiring-polygons` → `add-urban-sea-variants-and-parts-export` → `improve-finish-fidelity` → `add-vercel-deployment` の change 群を OpenSpec で完走（タイムラインは [[#経緯]]）。
...
### Base-variant finishes（multiply で再現できない仕上げ）

動機は **multiply 系の合成では再現できない仕上げの素材違い**（キッチン天板の大理石パターン、レンジフードの艶感、玄関床の素材違いなど）。`add-base-variant-finishes` change で実装:
...
### Even-odd マスク rasterizer & hit-test（multi-ring / 穴）
...
**Color-mode `tintBase`（ラベルレス、`shading_<id>.png` を直接 lift）**: サッシ枠 ⑰ は color-mode（multiply の元になる shade そのものが暗すぎて白系がくすむ）。texture-mode の `tintBase` と一本化したくなかったので **同じ JSON ファイル内で `label` の有無で 2 系統を切り替える**ハイブリッドを採用:
...
### Polygon schema の進化（v0 → v1 → v2）と境界正規化
...
**Crash バグ — legacy parts.json の `firstOuter` undefined**: Phase A 単体 commit 後に `/dev/trace` を開くと `Cannot read properties of undefined (reading '0') @ firstOuter`。原因は **古い v0 parts.json が API/draftStore 内部で v2 形に正規化されていなかった**こと。修正は **API + draft 境界で正規化**:
...
### Runtime variants スキーマ（urban-sea variant switcher）

アーバンシー sheet で Natural / Flat / Sharp の base-variant を runtime に切り替えられるようにし、texture-mode part が variant 切替に追従するパイプラインに揃えた（`add-urban-sea-variants-and-parts-export` ＋ `rewrite-seed-for-variant-keyed-workbook`）。
...
`~$部材リスト_*.xlsx` の `.gitignore` パターンは `b5890d5` で `.gitignore:55` に追加済で**ディレクトリ非依存でマッチ**するため、`resources/catalog/old/` 配下にも有効（`git check-ignore` で確認済み）。

## `/dev/trace` エディタ

### Sidecar drift 修正（バグ#1-3）

`/dev/trace` でポリゴン編集→保存しても、メイン画面では **更新前のポリゴン領域に色が載る** という drift バグ#3 を解消（`enhance-dev-trace`）:
...
Phase C では migration script `scripts/migrate-parts-multiring.mjs` を `parts.json` に適用、`npm run seed:masks` でマスク `.png` を全部再生成。続けてデザイナー（=セッションオーナー本人）が `/dev/trace` 上で **10 部材に sub-polygon ＋ 穴を実編集**（`b26cb83`）。⑨ スポットライトは 4 頂点プレースホルダ矩形 → **30 頂点**で天井トラック形状をトレース（bbox 414-2427 × 0-727、PR #5）。`mask_09.png` も sidecar 経由で自動 regen。

### UX 微調整 — 頂点 radius と右クリック削除バグ
...
## Excel export — variant-aware default
...
### プラットフォーム選定（`add-vercel-deployment` 提案時）
...
- **archive 時の post-implementation correction セクション**: 実装で判明した仕様逸脱や前提変更 (Vercel FS read-only など) を design.md に **後追い注釈** で残す。change を archive すると design.md は spec/ 配下の正本にコピーされないので、design.md 末尾に追記するのが breadcrumb として機能。例（`add-vercel-deployment` archive 時）:
  > Vercel filesystem is read-only → エディタの編集はブラウザ localStorage に保持 → ダウンロード → commit ワークフロー
- **"明示的非ゴール" の扱い**: tasks.md にチェックマーク付かないままで archive する判断 (custom domain / Password Protection / Analytics / build concurrency は将来別 change で個別検討)。`/opsx:archive` 実行時に「未完了タスクあり」警告を踏み倒すための **意思表示 callout** を tasks.md 末尾に書く運用
...
フェーズ② 残（当時見積）: `improve-finish-fidelity` 56–80 h（multi-ring 30h / per-option custom 6h / AI spike 16h / impl 16-24h / bg fix 4h） + Vercel 4–8 h。

**AI assist 効率**: 単独 11.5 h セッションで人月相当の進捗（コード＋仕様＋アーカイブ管理）。

### 運用コスト試算（OVERVIEW_JA.md §8）

`openspec/OVERVIEW_JA.md` に capability 8 本の現行仕様、進行中の提案サマリ、工数総合表、社内向け運用コスト試算を統合（§1–§5 現行仕様 / §6 提案サマリ / §7 工数 / §8 運用コスト）。§8 の要点（社内設計者・PM・営業利用、画像 Export 1 日 100 件想定）:
...
- **2026-04-25 頃** — MVP 初回 push（`b9b297b`、77 files / 12,387 行）。spec の合成順誤り 3 件を実装中に発見・同時修正
- **2026-04-27** — `add-base-variant-finishes` を実装〜archive 完走、`enhance-dev-trace` の drift バグ#1-3 修正、⑨ 30 頂点トレース（PR #5）。PR #8 で `improve-finish-fidelity`（5 項目: ①複数ポリゴン ②穴 even-odd ③オプション別テクスチャ `customTextureUrl` + `seed:custom-textures` + `seed:parts` 保護 ④生成 AI inpainting spike (SD/ControlNet で 5 サンプル生成→go/no-go) ⑤背景色補正 (顧客 re-render 依頼、代替 sharp `.modulate()` 1 行)）と `add-vercel-deployment` の 2 提案を起票
- **2026-04-28** — `openspec/OVERVIEW_JA.md` 新規作成（全体仕様 + 提案サマリ + 工数 + 運用コスト）
- **2026-04-29** — `add-multiring-polygons` Phase A/B/C（improve-finish-fidelity Item-1）。28/32 タスク完了、`proposal-memos-fidelity-and-vercel` ブランチに 4 commit push（`bbcdf4c` / `a711b38` / `22468c7` / `b26cb83`）。残 4 タスクを次セッション送り: ① multi-ring 専用 Playwright smoke（穴領域は色が載らないことの screenshot diff）② `firstOuter` 等の段階的廃止（`polygons.flatMap(p => p.outer)` 形に置換）③ `/opsx:archive` ④ README / AUTHORING.md の v2 schema 記述更新
- **2026-04-30** — urban-sea variant switcher + 部材リスト.xlsx 列構造リライト（`add-urban-sea-variants-and-parts-export` ＋ `rewrite-seed-for-variant-keyed-workbook`）。コミット 9 本（`2fc870b` → `6c9fcac`）
- **2026-05-01（早朝）** — `improve-finish-fidelity` 35/35 完走 → archive（tintBase `lift` / ② noEffect 拡張 / export variant-aware default / dev-trace UX 微調整）。`add-urban-sea-variants-and-parts-export` も同セッション archive。9 commit を origin に push（`c5ab76b` archive urban-sea / `b5890d5` archive improve-finish-fidelity — 228 files、+2810 / −740、LFS 35 MB アップロード）。Archive 先: `openspec/changes/archive/2026-05-01-improve-finish-fidelity/`、spec delta sync: `finish-spec-catalog`（4 ADDED）+ `presentation-canvas`（1 MODIFIED）。残アクティブ change は `add-vercel-deployment` (0/21) のみ
- **2026-05-01（続）** — `add-vercel-deployment` 着工（`15fae13` / `5cbda54`）。未解決で持ち越し: preview URL 上で `/dev/trace` autosave が「ローカルに保持中（再送信を試行...）」を継続表示、`/api/dev/parts` の preview 環境疎通確認が次セッション
- **2026-05-03（10:16–10:30 JST）** — `add-vercel-deployment` 完走。PR #8 を merge commit 方式で main に取込（`0e0bc75 Merge pull request #8 from meguruit/proposal-memos-fidelity-and-vercel`、過去 PR #3-#7 の規約踏襲）→ production 3.5/3.6 検証 → `/opsx:archive`（commit `acd6714`）。既存 `openspec/specs/dev-trace-tool/spec.md` の "Tool scope and gating" requirement を改訂（preview/dev のみ有効化を明文化）、新規 capability `vercel-deployment` を追加（`openspec/specs/vercel-deployment/spec.md`）、change ディレクトリを `openspec/changes/archive/2026-05-03-add-vercel-deployment/` に `git mv`。section 5 の 4 タスク（custom domain / Password Protection / Analytics / build concurrency）は明示的な非ゴールとして未チェックで archive。branch `proposal-memos-fidelity-and-vercel` を local + remote から削除

## Links

- [[05_learn/woodone-pboard-architecture]] — 出発点となった Woodone /pboard/ の調査
- [[05_learn/vercel-path-to-regexp-v6]] — Vercel デプロイの落とし穴（切出し先）
