...
| メール送信 | **Gmail API + Domain-wide Delegation (キーレス DWD)**（SMTP Relay から切替 → 詳細は下記「メール送信」節）。`Notifier` 抽象で吸収済み、置換はモジュール 1 つで完結。SMTP Relay 案は archive 扱い（user が Workspace 管理者本人で 2SV を 1 サービスアカウントにも入れたくなかったため鍵廃止に振り切り） |
...
## メール送信 — Gmail API + Domain-wide Delegation（キーレス DWD）

SMTP Relay → Gmail API + DWD への切替（2026-05-21）。`iamcredentials.googleapis.com` 経由で `developer1@` の short-lived token を発行 → `gmail.users.messages.send`。理由と手順は [[03_work/todobot]] §「2026-05-21 E2E 疎通の流れと詰まりポイント」で詳述。要点:
...
- **2026-04-28** — OpenSpec planning。23.0 人日見積で `line-todo-bot-mvp` 起票
- **2026-04-29〜30** — §1-§4 + B1-B4 + ops docs の 5 commit が landed（1 セッション完走、54 tests pass）。4-30 朝に運用ドキュメント二層化を確立
- **2026-05-11 時点の進捗**（残は §5-§10）:
...
- **2026-05-21** — メール送信を SMTP Relay → Gmail API + DWD に切替（OQ5 close）、@mention 解決バグを修正（`b83d678`）
