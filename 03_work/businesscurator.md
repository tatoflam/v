...
めぐる組業務メールを Claude サブエージェントで 4 カテゴリ
(`projects` / `clients` / `vendors` / `knowledge`) に自動分類し、
...
- **リポジトリ**: `~/repo/github/Bizuayeu/Plugins-Bizuayeu/BusinessCurator` (現在 `jooto-integration` ブランチ)。親プラグイン bundle は `Plugins-Bizuayeu`
- **サブモジュール群** (本リポ内):
...
- **分類器の運用状況**: 2026-06-16〜19 (run-89〜run-97 相当) で計 60 件超の分類イベントを観測 (経緯参照)。分類精度は概ね妥当だが「Proforma Invoice → projects (本来 vendors)」「LeTech 同一メールの projects/vendors ゆらぎ」の軽微な揺らぎあり
- **既知の問題**: `auto-ingest-push.sh → claude → SessionEnd → auto-ingest-push.sh` の再帰チェーンで `WIKI_INGEST_WORKER=1` 防御が伝播せず runaway worker loop が発生 (6/16 朝、concurrent workers 39 件 = PID 82847-92829、`hook-errors.log` に `git push failed` 3 件 = 03:32/06:47/06:48 UTC)。**lockfile (flock) 化が hook 側の対処候補**
- **BC ↔ MeguruWiki の役割境界**: BusinessCurator (本リポ) は受信メール → 4 カテゴリ分類 + Jooto 連携 + 案件メタデータ整備を担当。absorb 済み案件・vendor・知見ナレッジの canonical shard は別 vault [[03_work/meguru-wiki]] (`~/repo/github/Bizuayeu/MeguruWiki`) に集約される。BC の SessionEnd フックで enqueue されるサブセッション群は `/wiki-ingest` 経由で本 vault (tatoflam/v) に観測されるが、業務ナレッジ本体は MeguruWiki 側に蓄積される (= 2026-06-16 大規模吸収で実証、詳細 [[03_work/meguru-wiki#経緯]])

## 決定事項

- 2026-06-16: 個別メールは subject だけ wiki に載せる — 1 メール = 1 業務イベントだが、長期 PKM の観点では「テーマ × 頻度」があれば十分。詳細 audit は `~/.claude/wiki/state/ingest-log.jsonl` を参照
- 2026-06-16: lockfile (flock) 化を hook 側の対処候補として別件起票候補に — runaway worker loop (`94b63a5c` で初診断、run-90 の "rate-limit ループ" の真因) への対策
- 2026-06-17: 分類ゆらぎ (同一メールの二重 enqueue + 異分類) は両方記録して残す — LLM 出力の非決定性は受容。BusinessCurator 側の session_id ベース dedup は導入候補に留める

## 手順・Runbook

### ビジネスメール分類エージェント (= LLM サブエージェント 1 ターン分類)

受信メールを 4 カテゴリへ Claude サブエージェントで分類する:
...
### 2026-06-16: run-89 — 通常運用 4 メール分類
...
ワークフローが走った、という業務イベント観測。

### 2026-06-16: run-90/91 — 追加バッチ 20 件 (累積本日 24+ 件)
...
- **run-91 (user 手動起動、15:48-15:54 JST)** — run-90 と並走、追加で 13 件
...
### 2026-06-16: run-93 — 追加バッチ 12 件 (累積本日 36+ 件)

run-92 後の queue 88 件先頭 12 件を run-93 (18:16 JST、user 手動) で
...
### 2026-06-16: run-94 — 追加バッチ 12 件 (11 classifier + 1 self-stop、累積本日 47+ 件)

run-93 後の queue 98 件先頭から BusinessCurator 12 件を run-94 (18:34 JST、user 手動、cwd=`habi-bff`) で drain。内訳 = 1 wiki-ingest-meta (自己停止) + 11 classifier:
...
### 2026-06-17: run-96 — 追加バッチ 12 件 (8 classifier + 4 wiki-ingest-meta)

run-95 (= 6/16 18:46 JST) 後の queue 88 件先頭から BusinessCurator 12 件を run-96 (6/17 09:00 JST、user 手動、cwd=`meguru-manuals`) で drain。
...
= **run-96 直後 queue 残 76 件** (= 88 − 12)。BusinessCurator burst 残 ~54 件は次ラン (run-97) で継続消化。

### 2026-06-19: run-97 相当 — 追加バッチ 13 件 (3 classifier + 10 wiki-ingest-meta self-stop)
...
- [[03_work/meguru-wiki]] — 中央ナレッジ Vault (BC との役割境界は「現在の状態」参照)
