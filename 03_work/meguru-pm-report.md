...
`/opsx:propose` 一発で `add-ordinance-status-tracking` を起票し、design / specs delta / tasks 全生成 → 実装 → archive → branch `feat/ordinance-status-tracking` を 2 commit で push、[PR #2](https://github.com/meguruit/MeguruPMReport/pull/2) 開いてレビュー後 merge 完了。`b6b6fc7 Merge pull request #2` を `origin/main` に取り込み、マージ済みブランチをローカル削除 + remote prune まで一気通貫。`jooto-writer` プラグイン (= PR #1) に続く 2 本目の OpenSpec 1-session 完走パターン。

### 機能の全体像

Jooto 各案件プロジェクトの「申し送り」リストにある **条例・協議・申請関係タスクのチェックリストのチェック状態** を取得し、その結果を 2 つの場所に流す:

1. **進捗管理表_master (Google Spreadsheet)** の各案件行 × 条例列ヘッダ (省エネ申請 / 景観条例 / 狭隘協議 / 中高層申請 / 清掃局協議 / 緑化・雨水対策 / パース承認 / 躯体積算依頼) で構成される **右側「実績」チェックボックス列** を、Jooto チェック状態に応じて毎日同期 (= 各項目 2 列構成のうち、左の「日付」列は触らずに右の「実績」列のみ更新)
2. **weekly-report** に「条例・協議・申請関係の状況」マトリクス (= 行=案件、列=条例関係の列ヘッダ、レポート対象週内に状態変化のあった案件) を新ブロックとして掲載、HTML は `<table>` 整形描画

### 運用フロー (= 2 経路)

| 経路 | コマンド | スコープ | 想定タイミング |
|---|---|---|---|
| **日次 (自動)** | `jooto-backup --scope ordinance` で Milestone 実績を毎日同期 | 条例関係 8 項目の writeback のみ | LaunchAgent で毎日定時 |
| **週次 (手動)** | `/weekly-report` 内に「条例・協議・申請関係の状況」ブロック追加 | 週内変化案件のマトリクス | 週次レポート時に都度実行 |

### OpenSpec 完走の内訳 (= 1 session)

- `archive 2026-06-16-add-ordinance-status-tracking` (= **+7 added / ~5 modified specs delta**)
- `validate --all --strict` **15/15 PASS** (= 既存 spec との整合性確認込み)
- 2 commit: 1 本目 feature (37 ファイル staged、`docs/` は事前から未追跡のため除外)、2 本目 report + state
- push `origin/feat/ordinance-status-tracking` → PR #2 → review → merge `b6b6fc7`
- ローカル後始末: `main` 同期 + マージ済 branch 削除 + `git remote prune origin`

### docs/ の扱い (= 引き続き未追跡保持、jooto-writer PR #1 と同じパターン)

- `docs/` は 6/3 ベースの先行提案ディレクトリ (`.DS_Store` 同梱) で本実装と直接関連なし、commit からは除外して未追跡のまま保持。これは PR #1 jooto-writer 時と同じハンドリング (= 命名は別、扱いは踏襲)
- session 終了時点で git status の `docs/` は **未追跡のまま**、`reports/2026-06-16` のみ commit 2 に含めて push

### 設計のキモ

- **「左列は触らない」境界**: 進捗管理表 master の各条例項目は「日付 (左) / 実績 (右)」の 2 列構成。日付は人手入力のメタなので automation スコープから外し、実績ブール値だけ Jooto チェック状態と同期 = **片側 writeback 設計**で運用裁量を残す
- **scope 引数で writeback 範囲を絞る**: `jooto-backup --scope ordinance` で条例 8 項目のみ更新、他の Milestone 列に影響しない (= 既存 add-writer-catchup-sync の `--scope` パターン継承)
- **週次レポートは行/列 zip ロジック**: 進捗管理表の列ヘッダ 8 種 × 週内変化のあった案件行を zip して HTML テーブル生成 = bff 側の jooto-absorb と同型の zipping 設計

### OpenSpec フル 3 段 1 session 完走パターン (= 本 session が 2 本目)

- 6/16 既に jooto-writer (PR #1 MERGED) で初回完走 → 同日 17:48 起動の本 session で 2 本目完走 = **「propose → archive → PR → merge」を 1 session で回し切るパターン**が標準化しつつある
- ハーネス制約: 本番 deploy / 外向き curl は引き続き hard-gate、`gh pr create` までは skill 内 OK、merge は user 側 1 操作

### see also (本セクション)

- session `9b169945-6ca1-4412-9b5d-6255708d0b9a` (本 session、17:48 JST 起動、3.2MB transcript)
- [[02_diary/2026-06-16#18:42 JST  run-95]] (= 本セクションの run-95 diary 着地)
- [[#2026-06-16 jooto-writer プラグイン新設 + /jooto-template-broadcast skill + PR #1 MERGED (session 2de8fa6b、~1h42min)]] (= 6/16 1 本目の OpenSpec フル完走、本 session が踏襲)
- [[#2026-06-16 add-writer-catchup-sync archive + reports 06-09/06-12 commit + 2 commit push (session ca23a43a、10:10-10:14 JST)]] (= `--scope` writeback パターンの源流)
...
夜サイクル単発セッション (`cc512662-ef94-49eb-88ea-16f6b1448c3c`、JST 6/9 23:26 → 6/10 07:53、278 turn) で **memory `feedback_weekly_report_full_set` の 0〜9 ステップを通し実行** → `2026-06-09` 号レポートを Firebase + Drive の二系統に publish 完了。user 第一声「dailyのサイクルを回した上で(その時点ではメールを通知せず)最新のレポートを作成して、その上でメール通知できる？」に対し、**運用順序の整理** = (a) `--skip-email` 付き daily を先に回して state を温める (b) その上で weekly-report 0〜9 を実行 (c) 翌朝 09:00 の通常 daily が新 state を踏まえて email 配信、という三段で合意。

### 配信物 (= 06_output 着地対象)

- **Firebase HTML viewer**: `https://meguru-pm-report.web.app/r/2026-06-09/` (= GWS ドメイン制限あり)
- **Google Drive Doc**: `https://docs.google.com/document/d/1isvWiu0QKju2oXSb3TCI-_zhxH4CW1-idgVtBEWAheg/edit?usp=drivesdk` (= doc_id `1isvWiu0QKju2oXSb3TCI-_zhxH4CW1-idgVtBEWAheg`)
- ローカル: `reports/2026-06-09_weekly_update.md`

### 実行内容のサマリ

| 観点 | 内容 |
|---|---|
| 期日超過 | **3 件 (FY26_02 / 14 / 19) → 0 件**、すべて 6/5→6/9 で解消 (projects.csv 内全 24 案件で overdue zero) |
| Milestone 書き戻し | **5 セル全件 FY26_22 西巣鴨駅前** (`progress_checklist` x4 + `task_prefix_fallback` x1)、いずれも FALSE→TRUE の catchup |
| フェーズ遷移 | **FY26_22 西巣鴨駅前 「前提整理」→「意匠着手」** (= 本サイクルの最大イベント) |
| Jooto 進捗状況タスク取得 | **21/24 案件** (= projects.csv 24 件中 21 件)。未設定 3 件: 東尾久4丁目 / 中野区本町5丁目 / きつね塚通り前 |
| overdue 全体 (projects.csv 外含む) | 25 件 (= `FY26_01` のみ projects.csv 外で残存) |

### Phase 構成

- **Phase 0-3 (JST 23:26-23:54)**: user 確認 → memory full-set 0〜9 起動 → Step 1 `/jooto-backup --all-active --force` ✅ → Step 2 `/jooto-overdue-scan` 25 件 ✅ → Step 3 `/pm-master-backup` (Milestone + integration + handover + evaluation) ✅
- **Phase 4-5 (JST 23:54-)**: Gmail 24 案件並列取得 (バッチ 1: 8 案件) → 構造データ整理 → レポート本文ドラフティング
- **Phase 6-7 publish 中断 (JST 6/10 01:13-01:17)**: Step 8 publish が **Firebase 認証期限切れで停止** (= Step 0 で OK だったが約 1.5h 経過で失効)。`firebase login --reauth` がインタラクティブのため **user に手動 reauth 依頼** → user 再ログイン後、publish 再実行で deploy 成功
- **Phase 8-9 (JST 6/10 07:50-07:53)**: latest_state.json 更新 (FY26_22 = 意匠着手) + Milestone シート catchup 5 セル書き戻し完了 → `state/latest_summary.md` 更新 → 朝 9:00 の通常 daily が新 state を踏まえて email 配信する前提で終了

### user 第一声の前提整理 (= 運用ルール明確化)

dailyのサイクル (= color塗り + snapshot + state 更新) を **email skip 付き**で先に回し、その後 weekly-report を作るのが運用上正しい — 理由:
1. weekly-report は最新の Milestone シート / Jooto 進捗状況 / state/latest_state.json を「直前の通常運用ライン」が回した状態で参照したい (= データの単一ソース化)
2. 通知メールは weekly-report 着地後の最新 state を踏まえて朝 09:00 daily が出す方が一貫性が保たれる (= 夜に飛ばすと「朝の daily で発覚した差分」が翌朝の通知に重複する)

→ 本ラン以降の運用フロー = **夜 daily (skip-email) → 夜 weekly-report 0〜9 publish → 翌朝 daily (normal、email)** が canonical。

### 設計のキモ

- **Firebase reauth 中断は publish の予期される失敗モード**: Step 0 (= 認証チェック) を通った直後でも 1〜2h 経過で session expire しうる。Step 8 publish 直前にも軽量 reauth check を入れる余地あり (= 本セッションでは手動 reauth で回避、change 化はしていない)
- **5 セル全件 FY26_22 への集中**: 西巣鴨駅前 1 件で `progress_checklist` 4 + `task_prefix_fallback` 1 = 5 セル → 1 案件単体の catchup が大きく、`writeback` の **per-project ダイジェストを report 末尾に追加**する余地あり (= 本ランでは個別出力なし、ログのみで完結)
- **gmail-grabber は `/weekly-report` の入力ではない**: user の混乱 (= 事前に gmail-grabber を回す必要ありますか?) に対し、Gmail MCP `search_threads` + `get_thread` が直接読むため不要と明示。gmail-grabber は `.eml`/`.mbox` バックアップ別系

### MEMORY 化された / 関連する運用ポリシー

- **既存 `feedback_weekly_report_full_set.md`** (= 本ランで初の 0〜9 通し実行) — `/jooto-backup --force` + `/jooto-overdue-scan` + `/pm-master-backup` + Gmail 取得 + report 生成 + publish + writeback + summary 更新の 10 ステップ
- **新運用ルール (= 本ランで確立)**: 夜 weekly-report の前に「`alert_email_schedule: manual` 相当の日次サイクル」を先に回す。実態は `--skip-email` 付きの daily kickstart + `/jooto-backup` の組合せ

### 残課題

- **Jooto 進捗状況未設定 3 案件**: 東尾久4丁目 / 中野区本町5丁目 / きつね塚通り前。Milestone catchup のソース不在 = 次サイクルでも `progress_checklist` 経由の書き戻し対象外。PM 側に「進捗状況」タスク作成依頼を検討
- **Firebase reauth の半自動化**: Step 0 認証チェックを Step 8 直前にも入れるか、`firebase login:ci` 化を検討 (= 本セッションでは手動で回避)

### see also (本セクション)

- [[02_diary/2026-06-09#23:26 cc512662]] — 本セッションの diary 着地
- [[06_output/2026-06#Firebase Hosting — 週次レポート HTML viewer (2026-06-09 号)]] — Firebase URL カタログ
- [[06_output/2026-06#Google Drive Doc — 週次レポート Google ドキュメント (2026-06-09 号)]] — Drive doc URL カタログ
- [[03_work/meguru-pm-report#2026-06-05→07 milestone-alert 2 change]] — 直前の milestone-alert change 完了から続く運用線
...
## 2026-06-11 weekly-report 順序バグ初回観測 (session 1bd39308)

6/11 朝のフルセット run で初めて発生。**症状**: `/weekly-report` 生成 → `/weekly-report-publish` で配信した工程グリッドが Jooto の最新チェックを反映していない (= 「フルセットの割に妙にコンテンツが少ない」が出ない、初回ランでは発覚せず、後段 2378c86f で本格 root cause 確定)。

### 本セッションでの取り組み

`/weekly-report` 0〜9 のフルセット (= memory `feedback_weekly_report_full_set.md` 定義) を `firebase login --reauth` 経由で完走 + Milestone 12 セル書き戻し。
- run 範囲: jooto fetch (`--all-active`) → overdue scan → PM マスタ → baseline sync → `/weekly-report` → publish (Drive) → Firebase deploy → Milestone 書き戻し
- 完走時の状態認識: 「Milestone writeback (12 セル FALSE→TRUE) は publish 後に走るのが現運用」と判定 (= 後段 6/12 で誤判定だったと確定)

### 残った違和感 = 6/12 への引き継ぎ

publish の中身を grid で見たとき、Jooto で既に done の項目がいくつか未反映に見える違和感が残ったが、root cause は **6/12 朝の 2378c86f で「order が逆」と確定**するまでは「Jooto fetch のタイミング差」と誤推論されていた。順序バグの根本的な指摘 + memory 更新は 6/12 のセッションで実施 (= 詳細は次 H2 参照)。

### Pages touched (本セッション)

- 配信物: 2026-06-11 号 (= Doc + Firebase) — **冗長化版**、6/12 修正版に冪等更新で上書きされた (URL 不変)
- memory: `feedback_weekly_report_full_set.md` (= 順序記述は触らず、誤りに気付いていない状態で完走)

see also: [[02_diary/2026-06-11]] / [[03_work/meguru-pm-report#2026-06-12 weekly-report 順序バグ根本対応 + 修正版再配信 (session 2378c86f)]] (= 翌日に root cause 確定)

## 2026-06-12 weekly-report 順序バグ根本対応 + 修正版再配信 (session 2378c86f)

6/12 朝の再走で user が初見指摘:
> 「フルセットの割に、妙にコンテンツが少ないね。本当にあってる？大原は本受になり、jooto の進捗状況もチェック済なのに、なぜ工程軸の状況が更新されないの？何か根本的な問題があるように見えるので、もう一度見直してくれない？」

### root cause 確定

「**6 書き戻し → 7 Milestone 再取得 → 8 工程グリッド再生成 → 9 publish**」の **正しい順序**が memory `feedback_weekly_report_full_set` に既に記載されていたにもかかわらず、私が「weekly-report → publish → writeback」の順で publish を先に走らせて違反 → 工程グリッドが書き戻し前の状態で固定 → **1 周遅れの内容で配信**。

= 仕様バグではなく運用バグ。memory には正しいシーケンスが既に書かれていたが、運用時に手順を守らなかった失敗。

### 修正対応

writeback (Jooto → Milestone) → Milestone 再取得 → 工程グリッド再生成 → 修正版を冪等 publish。Doc / Firebase の URL は不変 (= 既 publish の冪等更新で 6/12 配信先に上書き)。
...
| 世田谷大原 | 図面整合性確認 → **施工着手** |
| 中野1丁目 | 構造着手 → 仮受付 |
| 西荻北1丁目 | 前提整理 → 構造着手 (8 工程キャッチアップ) |

全体サマリにフェーズ遷移 4 件 + 書き戻し概要を明記、`state/latest_state.json` の phase も更新。

### 再配信先 (= 6/12 号、冪等更新)

- Firebase HTML: `https://meguru-pm-report.web.app/r/2026-06-12/` (Meguru GWS ドメイン制限)
- Google Doc: `https://docs.google.com/document/d/1KsIPB-wIHD2xvTL-baitN6-IrVwHh2ElxPY1Z2_qRFU/edit`
- Markdown 原本: `reports/2026-06-12_weekly_update.md`

### 再発防止

memory `feedback_weekly_report_full_set.md` の冒頭に「**publish は最終ステップ**」を明示追加、MEMORY.md 索引の hook も「順序厳守」が伝わる文言に強化。同 session 内で 2 回 memory edit (= `Edit` 経由)。

### 副産物: D008 weekly publish の別 bug 発見 (= 09:00 fire で `today_count_before=0`)

本ラン中に user 別話題で発覚: ThreadsPosts の 6/12 12:00 fire 時、`published_at` / `has_affiliate` が D008 published file に書かれていなかったため `countTodayPublished` が 0 を返した (= 別 publish.js bug)。本ランスコープ外、別 change 起票推奨として flag。後段 ThreadsPosts session 8b819182 で publish-due-greedy-reorder change のスコープ判断材料となった。

### 学び (= run-82b で改めて記録)

- **memory に書かれていた正しい順序を運用時に守らなかった**: spec bug ではなく行動規律 bug。同じ轍を踏まないよう「publish は最終ステップ」を memory ヘッダで明示
- **「妙にコンテンツが少ない」は user の優れた spot check 質問**: 数値や表面的な OK / NG ではなく、内容の希薄さに違和感を感じる質的判断が root cause 発見の起点となった
- **冪等 publish の安全性**: Doc / Firebase の URL を変えずに修正版を上書きできる設計のおかげで、再配信のオペレーション負荷が低く保てた

see also: [[02_diary/2026-06-12]] / [[06_output/2026-06]] (= 6/12 publish 2 新規ブロック) / [[memory:feedback_weekly_report_full_set]] (= 順序厳守ルール強化)
