...
## 2026-05-22 → 06-07 — affiliate-first-research 完走 + PR #14 + W23 normal phase 初週 A REVIEW (session 0c5ca024、長期スパン)

セッション 0c5ca024-b170-475a-b15c-f05e11bfddce (ThreadsPosts 1815 KB、enqueued 2026-06-07T01:41:43Z = 6/7 10:41 JST に session end)。長期スパンで **05-22 propose → 05-30 apply+archive+commit → 06-06 PR open → 06-06/07 W23 weekly cycle A REVIEW done で pause** の一気通貫。

### Phase A: `/opsx:propose affiliate-first-research` (2026-05-22、warmup_w3)

問い: 「楽天アフィ起点のリサーチ動線をやりたい」。今の YouTube-seed 起点は研究テーマに収益化できる商品があるとは限らないので、**a.r10.to 起点で「収益化商品が確実に存在する」ところから逆算**する方が空振りしない。購買導線としても自然。

OpenSpec change `affiliate-first-research` 起票・全 4 artifact 完成・validate 済。6 つの技術判断:

| # | 判断 | 動機 |
|---|------|------|
| 1 | seed は products.yaml の a.r10.to 付きエントリのみ | 楽天ページ scrape を避けて TOS/ban リスク回避 |
| 2 | 楽天ページは scrape しない | ToS 違反予防、curated フィールドで足りる |
| 3 | 深掘りは Claude オーケストレーション (論文 + 既存ナレッジ) | 多ソース突合で広告臭を消す |
| 4 | Knowledge に**起点商品の逆参照を必須化** | 広告臭リスクを research-depth gate で機械的に防ぐ |
| 5 | monetization のデフォルト=3 行 hook + reply_chain + 末尾リンク | CLAUDE.md「monetization は長文 OK」と Threads 「原則 3 行」を reply_chain で両立 |
| 6 | 約 1/4 は reply_chain なし長文単発で variation | 「急に思いついた風」(人間らしさ)、`monetization_longform_ratio: 0.25` advisory |

### Phase B: `/opsx:apply affiliate-first-research` (2026-05-29→30、28 タスク完走)

**新規 3 module**:

- [`dept/dev/pipeline/research/affiliate_seed.js`](https://github.com/tatoflam/ThreadsPosts/blob/main/dept/dev/pipeline/research/affiliate_seed.js) — products.yaml から a.r10.to 付き seed 読込、genre フィルタ、curated フィールドからキーワード抽出 (**ネットワークアクセスなし**)
- [`dept/dev/pipeline/research/affiliate_research.js`](https://github.com/tatoflam/ThreadsPosts/blob/main/dept/dev/pipeline/research/affiliate_research.js) — `npm run research:affiliate` CLI、対話 seed 選択 → 補足キーワード → 深掘り handoff、seed ゼロ時は `route: youtube-seed-fallback` を出力
- [`dept/dev/pipeline/lib/research_depth_gate.js`](https://github.com/tatoflam/ThreadsPosts/blob/main/dept/dev/pipeline/lib/research_depth_gate.js) — monetization draft 作成前に「起点商品を引く**非商品**ソースの Knowledge が 1 件以上」を検証

**既存 6 module 改修**:

| ファイル | 改修内容 |
|---|---|
| research/index.js + topic_synthesize.js + write_knowledge.js | `--source-product` / `--source-affiliate-url` を K0XX Topic/Transcript の frontmatter に刻む |
| generate_post.js | monetization は「3 行 hook + reply_chain + 末尾リンク」をデフォルト scaffold、`--longform` で長文単発、`--source-product` で research-depth gate を起動 |
| inject_affiliate.js | reply-chain draft では `#PR 広告` + a.r10.to を**最終 reply** に付与 (親投稿には付けない)、500 字制限はリンク保持側で評価 |
| validate_frontmatter.js | `source_product` / `source_affiliate_url` の型検証 |
| posting_policy.yaml | advisory `monetization_longform_ratio: 0.25` |

**ドキュメント**:

- `weekly-cycle` SKILL の C-RESEARCH を affiliate-first **3 段構成** (C-0 seed → C-a 4 ソース深掘り → C-b K0XX 生成) に書き換え、fallback 併記
- `research-youtube` SKILL を「深掘りステージの一手段」に再位置づけ
- monetization テンプレ 6 本の `## 構造` に reply-chain デフォルト注記

**検証**:

- 全 572 テスト pass、lint:posts・templates・hooks・validate 全 pass、openspec validate `--strict` pass
- 試走 (fallback 経路、affiliate-first 経路 (temp products)、research-depth gate、reply-chain 生成 (lint+validate 通過)、PR ブロックが最終 reply に着地、`--longform` で reply_chain なし) — すべて確認済

### Phase C: `/opsx:archive` (2026-05-30、main specs sync)

`openspec/changes/archive/2026-05-30-affiliate-first-research/` にアーカイブ、main specs sync 完了:

- **新規**: `openspec/specs/affiliate-first-research/spec.md` (Purpose + 7 要件)
- **更新**: `openspec/specs/weekly-cycle-orchestrator/spec.md` の「Skill-based orchestration」要件を **affiliate-first 上流 + fallback 構成**に差し替え
- **追記**: `openspec/specs/post-templates/spec.md` に 3 要件追加 (monetization reply-chain default / PR-on-final-reply / longform variation)

**32 capability spec が `--strict` で valid**。commit [`46ffcb9`](https://github.com/tatoflam/ThreadsPosts/commit/46ffcb9) 「feat: affiliate-first-research (OpenSpec change)」(2026-05-30T00:04+0900、1 commit / 30 files) → `feat/affiliate-first-research` push。

### Phase D: PR #14 オープン (2026-06-06T02:17:17Z = 6/6 11:17 JST)

**PR #14**: [feat: affiliate-first-research (OpenSpec change)](https://github.com/tatoflam/ThreadsPosts/pull/14)
branch: `feat/affiliate-first-research` → `main`

warmup_w4 (2026-05-28〜06-03) 経由で 06-04 から **normal phase** 移行 (2 投稿/日、affiliate 80% 上限)。PR #14 は account_1 が normal phase に入った直後の affiliate-first 整備として open (merge 判断は user)。

### Phase E: `/weekly-cycle` ISO 2026-W23 (2026-06-06→07、A REVIEW done で pause)

account_1 が **normal phase 初週** (W23 = 2026-06-01〜06-07)。Step 0 状態判定:

- W23 サイクルは未着手、前週まで weekly_markdown は **W20 で stop**、metrics.csv は存在せず、published 10 / drafts 3
- W23 は月初週なので **B-3 persona review トリガ発火**
- A REVIEW は metrics.csv 不在で qualitative review

[`dept/marketing/strategy/weekly/2026-W23.md`](https://github.com/tatoflam/ThreadsPosts/blob/main/dept/marketing/strategy/weekly/2026-W23.md) Section 1 を埋めて **A REVIEW done で pause** (`status: draft` のままローカル保持、commit せず)。

**違和感サマリ 5 件** (Section 1 末尾に明示、B-MARKETING への論点):

| # | 違和感 | 状況 |
|---|--------|------|
| 1 | **9 日 posting gap** | 最終 publish 2026-05-28 D011、今日 06-06 で 9 日空白、normal phase 初日〜day3 が無投稿 |
| 2 | **3 drafts 19 日滞留** | D007/D008/D013 (informational + classic + image 付き) が `scheduled_at: null` で 2026-05-18 から放置 |
...
- weekly markdown: `dept/marketing/strategy/weekly/2026-W23.md` `status: draft` (commit せずローカル保持)
- 次回再開時の入口: **B-INTAKE**
- 持ち越し論点: 上記 5 件
- B-3 persona review が W23 month-start トリガで次回確定実施
- 元の優先順 step 3 (interior products 登録) / step 4 (/post-due) は次回以降

### 構造的観察

- **affiliate-first route の実用化ボトルネック**: products.yaml に interior 商品ゼロのため、現状は **fallback 経路でしか走らない**。normal phase で affiliate 80% まで使えるようになったが、interior 商品登録が無いと枠が余る。「次にやらないと無意味なこと」リストの最上位は **interior products 登録**
- **monetization reply-chain pattern の Threads 適合**: 親=3 行 hook で reach 取りに行き、reply chain で深掘り、最後の reply に PR + a.r10.to → 親投稿が広告臭くならない & reply 開かれた読者には十分な情報、という Threads UI と CLAUDE.md「monetization は長文 OK」両方を満たす形
- **research-depth gate の機能**: 起点商品を逆参照する Knowledge が**商品以外のソースを最低 1 件引用していないと draft を作れない**。広告臭くなるリスクを単一仕組みで防御 (D3 タスクで必須化)

### see also (本セッション)

- [[02_diary/2026-06-07]] — 17:00 run-69 entry (本セッション + 並行する meta drain 等)
- [[06_output/2026-06]] — PR #14 セクション
...
## 2026-06-07→08 W24 /weekly-cycle: buzz-first 運用が定着、user intake が天然 channel 化 (session 23ea12c6、~12h 跨ぎ)

### 入口

`/weekly-cycle` B-MARKETING / C-RESEARCH 段を回している最中に user が **非構造的に** 「これ投稿に活かしたい」を 3 件流し込んだ:

1. **DeLonghi Combi コーヒーマシン** — 楽天スーパーSALE 中 (6/11 まで)、user が実購入。「コーヒーと一緒に過ごす時間がより素敵になりそう」voice
2. **YouTube 人感センサー解説リンク** — interior の動線/照明テーマで使えるか？
3. **ビアグラス検証** — 別 buzz として inbox に貯めた小物

これに対し、生成系は使わず純粋に「receive → file → push」だけを実行 → buzz-first 運用が手順なしに天然に動いた状態。

### 着地

| 場所 | 内容 |
|---|---|
| `dept/affiliate/data/products.yaml` | DeLonghi スーパーSALE entry (W24 publish 必須、セール期限 6/11) |
| `dept/research/inbox/2026-W24/candidates/knowledge_seeds.md` | 人感センサー YouTube + ビアグラス検証 (= buzz pair) を pending entry として 2 件追加 |
| memory `feedback_threads_post_lightness_2026-06-08.md` | 「Threads post はあっさり voice、K0XX フレーズはアクセント 1 行」「3 軸 / 2 種類の体系化 voice は reply_chain でも禁止」 |
| `MEMORY.md` | 上記の index 行追加 |
| commit `17e324e main` (push 済) | 上記 4 ファイル 1 commit |

### 構造的意義

- **buzz-first 運用 = 暗黙手順なしの "user-as-channel"**: 専用 skill (= research-intake / buzz:adopt) を叩かなくても、`/weekly-cycle` の B-MARKETING / C-RESEARCH 段で user が「これも入れて」と言うだけで products.yaml + knowledge_seeds.md に正規 entry が落ちる。= 既存パイプラインが「user の自然 intake」を吸収できる形に成熟した
- **手数最小の "アクセント voice" 明文化**: 朝の K0XX 系 (= [[06_output/2026-06#GitHub PR — ThreadsPosts PR #14]]) で「フレーズ拝借型」を整備した直後に、本セッションで「あっさり voice」のガイドラインを memory に固定 → K0XX-style の体系化 voice が暴走しない歯止め
- **DeLonghi セール window = W24 publish 義務**: 6/11 まで → W24 monetization の確実な monetization slot = 翌日の `/post-due` (= [[03_work/threadsposts#2026-06-08 — W24 publish]]) で活きる

see also: [[02_diary/2026-06-07#23:31-翌朝]] / [[02_diary/2026-06-08#11:38 23ea12c6 finalize]] / [[06_output/2026-06]]

## 2026-06-08 — W24 publish: D016 monetization (DeLonghi 白) + D017 empathy 全自動 → 4 memory + 2 K0XX stub + hooks「将来の私へ」新設 (session a1953a4b、~3.5h)

### publish 結果

| draft | post_id | type | 結果 |
|---|---|---|---|
| D017 (empathy / SNS で照明予算オーバー) | `17901893604283150` | 親 + 2 reply | 全自動 publish ✅ (第 2 試行、1 回目は「楽天」mention で compliance NG) |
| D016 (delonghi monetization / 「白もある」) | `18097260743464713` | 親自動 + reply 3 本 **手動** | 親 + 手動 reply で完結 ✅ |

= **W24 monetization slot 1 (D016)** + empathy 1 (D017) = 2 publish。23ea12c6 で intake した DeLonghi セールを window 内で消化。

### 4 memory 追加 (= 本セッション = oheyamemo voice の輪郭が一気に鮮明化)

| memory | 教訓 |
|---|---|
| `feedback_non_affiliate_no_rakuten_mention_2026-06-08` | empathy / informational 等の **非 affiliate post で「楽天」 mention 禁止** (= EC platform 名は出さない)。monetization post は bio 透明性 + 「楽天アフィリエイト参加中」と整合するので OK |
| `feedback_voice_taigen_dome_ban_2026-06-08` | **体言止め多用禁止** + 「〜なんだよね」推奨。oheyamemo voice は自分語り型、体言止め三連を避ける |
| `feedback_voice_aa_bb_nandayone_pattern_overuse_2026-06-08` | 「AA、BB なんだよね」一辺倒も NG (= 体言止め回避で別 formula 化、oheyamemo 全 published 同型で恥ずかしい)。hook variant 5+ で系統 cross、同 pattern 連続 3 回禁止 |
| `reference_threads_api_reply_chain_failure_2026-06-08` | Threads API **reply container が「メディア not found」で反復 fail** (carousel / 単一画像 / 画像なし共通)。3 connect 試行で fallback、3 回 fail で **手動 reply 経路 or delete+retry / carry-over** |

### Library 拡張

- **hooks「将来の私へ」category 新設**: [@benri_na_kurashi](https://www.threads.net/@benri_na_kurashi) buzz から借用、`dept/content/hooks/interior.md` に追加。「過去の自分と未来の自分」 perspective 軸
- **K014 (coffee × interior)** + **K015 (食卓道具 + 将来の私へ voice)**: 新規 Knowledge stub。K014 は本セッションの DeLonghi publish の knowledge backing、K015 は次サイクル D018 用 (ピッチャー)
- **products.yaml**: `r10-h50phd` (ピッチャー) seed 追加

### W25 carry-over

- **D015** (wltech 観賞用シーリング): 翌夕 18:01 JST [[reference_post_due_launchd_schedule|launchd]] 窓で自動 publish 予定
- **D018** (将来の私へ + ピッチャー): status: draft、image 未取得、scheduled_at 未設定。W25 で finalize
- **D007 / D008 / D013**: 素材保持、reply chain 組み込み待ち (= heavy draft の rework 完了せず)

### 構造的意義

- **「W24 で publish を回す」目標達成**: 朝 23ea12c6 で intake した DeLonghi を午後の `/post-due` で publish まで通した一気通貫。buzz-first → research → content → publish 動線が **1 セッション内で短縮接続** (= /weekly-cycle 7 phase を朝 ~ 昼で物理的に圧縮)
- **publish 体力 vs voice 整備のトレードオフ**: 「publish の体力消費がだいぶ大きかったセッションでした」と user が明示 = 1 publish-day に 2 post + 4 memory + 2 K0XX stub + 1 hook category を詰めるのは持続可能性が低い → W25 以降は「memory / hook 整備は別セッションに切る」「publish session は publish のみ」の分業を考えるべきライン
...
「デロンギ投稿を最後に、定時の `--due` post が止まっている気がする」観察 → 6/10 16:00〜6/12 12:00 の 4 連続 fire がすべて `gate_rejected` (= affiliate 比率超過で D008/D015 がブロックされた) と判明。手動 1 件解除 + 自動化レイヤとして **3-bucket greedy reorder** を `/opsx:propose` → `/opsx:apply` でフルセット起票 → push 完了 (= `merge` 済確認 = 既 main 反映)。

### 詰まりの根本原因

- `--due` ループは `scheduled_at` 昇順 1 passで evaluate していたため、最初に当たった affiliate 投稿で phase_gate 1.00 > 0.80 を引いてしまうと **non-affiliate 候補が後段にあっても順番が来ない**まま fire が終わる
- N! 全探索による組合せ救済は計算量爆発 (= candidates_n=20 で 2.4×10¹⁸)、かつ phase_gate の評価コストが decision あたり LLM ゼロでも数百 ms

### `publish-due-greedy-reorder` change (4 artifact + 17 task) の解

3-bucket 分類で投票:

| bucket | 中身 | 優先度 |
|---|---|---|
| `resume` | `status: scheduled` 中で前回 fire 時に gate_rejected 履歴あり | **1st** (詰まり優先解消) |
| `non_aff` | non-affiliate (= 配信比率 floor 安全側) | 2nd |
| `aff` | affiliate (= phase_gate に引っかかる側) | 3rd |

- 各 bucket 内 tie-break: `(scheduled_at, filename)` 昇順
- **N! 探索を退ける**理由: 3-bucket greedy で詰まり解除には十分、決定的、log の解釈性が高い
- **人ゲート不可侵**: `status: draft` は admission からスキップ (= mutation なし)

### `reorder_decision` ログ 1 行 invocation あたり 1 回必ず emit

```
[publish] reorder_decision=true candidates_n=N
  buckets_{resume,non_aff,aff}=[...]
  chosen_order=[...]
  reorder_changed=<bool>
```

= log 1 行で「何が candidates だったか」「どの bucket に振り分けたか」「実際に何を選んだか」「reorder で順序が変わったか」が後追い可能。**最小冗長で全状態が見える**設計。

### 実装着地 (`dept/dev/pipeline/lib/due_reorder.js` 新規 + `publish.js` 差し替え)

- `selectAdmissionOrder` helper (純関数): admissible 集約 → bucket 分類 → reorder
- `publish.js` refactor: pre-pass で admissible 集約 → reorder → 既存 phase_gate 逐次判定
- 19 unit + 44 既存 publish/phase_gate test 全 pass (= regression なし)
- 既存 2 fail (`image_license_schema` / `validate_frontmatter.image_license`) は **pre-existing failures**、reorder change と無関係を AST 検証

### 別 bug 発見 (out-of-scope follow-up)

D008 publish 後の published frontmatter に `published_at` / `has_affiliate` が書かれていない → 6/11 19:00 fire で `today_count_before=0` を返した。これが 6/11 19:00 → 6/12 12:00 で D015 が再度 gate_fail した真因。`countTodayPublished` の信頼性に関わる **別 publish.js bug** で、reorder change のスコープ外。tasks.md §5 に記録、別 change として起票推奨。

### docs ・追記

- `docs/operations.md` 「Publish to Threads」行に reorder の言及追加
- `dept/dev/pipeline/__tests__/publish_reorder_manual_verification.md` 新規 (9 シナリオ + 一時 fixture 再現 recipe)
- `loop:check` への positive signal 追加は **skip 決定** → design.md "Decision Log" に理由記録 (reorder は判断ではなく自動化、loop の health gate を増やすメリットが無い)

### 状態

- main に 3 commit push 済 (= 本セッション内では実装 + テスト + docs を merge 完了)
- task 19/20 (`4.4` = 翌 12:00 / 19:00 launchd 実 fire 観測は post-merge user 確認に deferred)
- archive は次回 fire 観察 ok 確認後

### 副産物・学び

- **「人が判断する自動化」と「機械が決定的に並べ替える自動化」の境目**: `status: draft` の人手判断面には触れず、`status: scheduled` の発送順だけ並べ替えるという薄い切り口に絞ったのが効いた。OpenSpec の delta spec MODIFIED で `--due` ordering 仕様だけ書き換えれば、関連 capability (`local-publish-runner` の他 path) は touch しなくて済む
- **詰まり解除を greedy で 1 行ログ化する pattern**: gate_rejected 連鎖が起きた時に「何が candidates / どう振り分けた / 何を選んだ」が 1 行で読めると、深夜の oncall でも 5 分で原因把握できる。`reorder_decision=true buckets_aff=[D015]` のようなコンパクトな log 表現は他の publish 系 pipeline でも流用可

see also: [[02_diary/2026-06-12]], [[05_learn/openspec-retroactive-flow]]

## 2026-06-15→16 publish-pipeline-self-sustaining フル走り — published=0 loop 撲滅 + reply-chain flaky 根治 + 段階A live 復旧 (session 51140742、24h 跨ぎ)

### 入口

「3日前にポストしてるけど以降はポストがないね。スキップされてしまうケースをどれだけ除外できるかが鍵かな？1日5件程度はポストする方向に持っていきたいんだけど、まだ無理な理由と、どうすれば常時運用できるのか、プランだしをよろしく！」(2026-06-15 11:42 JST)

= 6/12 の 3-bucket reorder change で「順序」は解決したが、その後 **3 日連続で `publish:due` が `published=0` を返している沈黙** の正体がまだ残っていた → 構造的 deadlock を 1 session で根治。

### 沈黙の正体 (= published=0 ループの根本原因)

実態確認 (`publish:dry` 走らせ):

| draft | status | gate 判定 | 理由 |
|---|---|---|---|
| D007 | draft | skip | status=draft (人承認待ち) + W24 で独立 publish 凍結 |
| D013 | draft | skip | 同上 (素材は D015 reply に転記済) |
| D018 | draft | skip | status=draft + **画像なし** (monetization は画像必須) |
| D015 | scheduled・due | **gate_rejected** | 単独 affiliate 投稿 → ratio=1.00 > 0.80 で毎回 reject |

= publishable な候補が実質ゼロ。launchd は 12:00/19:00 JST にちゃんと発火しているが、毎回「1 本だけ admissible (D015) → ratio で蹴られる → published=0」を繰り返していた。

**特に D015 が構造的詰み**: `affiliate_ratio_max: 0.80` は「その日の affiliate 比率が 80% 超ならダメ」の意味なので、**affiliate 投稿を「その日の 1 本目」に出すと必ず ratio=1.00 で reject**。先に非 affiliate を 1 本通さないと永久に出せない構造。ペアにすべき非 affiliate 在庫 (D007/D013) は凍結済、D008 は消化済、誰も D015 を救えず毎日同じ reject ループ。

### 「1 日 5 件」が今は無理な層ごとの壁 (= 段階C で扱う事項の整理)

5 件/日 = 週 35 本の「scheduled かつ gate 通過するドラフト」が要る。今は事実上 0。壁を分解:

1. **政策上限**: normal phase `max_posts_per_day: 2`、baseline も 3。gate が 3 本目以降をハード reject
2. **affiliate 比率 0.80**: 5 本なら affiliate 最大 4 本。かつ「1 本目 affiliate」は常に詰む構造は volume 上げても残る
3. **在庫枯渇 (最大のボトルネック)**: 生成・schedule のスループットが追いついていない
4. **min_gap_hours: 6**: 6h × 5 = 30h で物理的に 1 日に収まらない、gap 短縮が必須
5. **launchd 1 日 2 発火のみ**: publish 窓が 12:00/19:00 のみ
6. **手動ゲート + flaky**: status=draft 人承認、monetization 画像必須、Threads API reply-chain flakiness

→ **5/日は「上限を上げる」だけでは到達不能**。本質は ①在庫スループット ②affiliate ペアリング構造 ③BAN リスク。account_1 は normal 入りまだ 11 日目 + 過去 BAN 履歴、急に 5/日へ跳ねるのは inauthentic 検知的にも危険 → **段階C は milestone gated で別 change** という結論。

### 解 — 3 段プラン

#### 段階A: 今週の復旧 (まず安定 2/日に戻す、コード変更ゼロの live 操作)

| task | 内容 | 結果 |
|---|---|---|
| 4.2 | D015 救出: 非 affiliate を 1 本「その日の 1 本目」に通してから D015 を 2 本目にペア。今すぐ出せる非 aff 候補が無いので **新規 empathy 短文 D019** を生成 | ✅ `5e9a0c8` D019 仕込み + `97f8c41` scheduled、同 19:00 JST fire で `published=2 / D019 ✅ 89imp / D015 partial` |
| 4.1 | D018 に画像付与 → status:scheduled に | ⚠️ 画像のみ完了 (`c17ed8c` Unsplash Masha Rostovskaya teapot 配置、mood image 注記)、**非 aff ペア確保が要る** ので status=draft のまま保留 |
| 4.3 | `publish:dry` で gate pass 候補が複数出るのを確認 | ✅ pairing 検証 OK、ratio 0.50 で D015 通過 |
| 4.4 | `npm run metrics:sync` で stale 解消 | ✅ `32f68dc` 14 rows sync、`da6f8b9` published frontmatter に metrics 反映 |

#### 段階B: 構造修正 (= `publish-pipeline-self-sustaining` change、6 group / 22 task / spec-driven)

| group | 成果 | 着地 |
|---|---|---|
| 1. `publish_stall` fault | `loop_continuity.js` に **5 番目の detector** (by-time 36h since last post / by-structure ペア不成立)。warn-only、publish を block しない。test 4 本 pass | `f35df12` |
| 2. ペアリング不変条件 | `rotation.js` に `validateRotationPairing`、`rotation_account_1.yaml` を **7 → 14 entry の weekday ペア型** (各 weekday = `[非 aff → monetization]`) に再設計。沼子型 + 隔週 mission 維持、decision log 追記。test 5 本 pass | `f35df12` |
| 3. 在庫バッファ invariant | weekly-cycle SKILL の A REVIEW gate を **4 → 5 detector** に、D CONTENT に **step 10「N≥4 日バッファ + ペア充足」ゲート**追加 | `f35df12` |
| 4. 段階A live 復旧 | (上記) | `5e9a0c8`/`97f8c41` etc |
| 5. ドキュメント | CLAUDE.md / operations.md / posting_policy.yaml / `memory:project_publish_pipeline_self_sustaining` 反映 | `f35df12` |
| 6. 検証 | lint 17/17 + 新規 test 9 本 pass + `openspec validate --strict` green | `f35df12` |

`loop:check` が実データで `publish_stall (warn): 96h since last post but 1 admissible draft — D015` を fire し、本根本原因確定のトリガになった = invariant が早期可視化を担うことを実証。

#### 段階C: 上限増 → 5/日 (= 別 change、milestone gated)

`max_posts_per_day` / `min_gap_hours` / launchd 窓の宣言変更が主。**安定 2/日が数週 BAN/警告ゼロで回ってから**起票。本 change の rotation は「各 monetization に非 aff 先行」不変条件を維持したまま日あたり本数を増やせる土台になっている。

### Threads API reply-chain flaky の根治 (= 6/8 D016 以来の累積 flakiness を 1 commit で潰す)

6/8 D016 (carousel)、6/9 (画像 1 枚)、6/16 D015 (画像なし) すべてで「親 publish OK / reply container が `メディア not found` で repeat fail」が再現していた既知の Threads API flakiness ([[reference_threads_api_reply_chain_failure_2026-06-08]] でメモリ化済) を、本 session で **`publishContainer` に container-not-ready の backoff retry を実装** → `5bc2a29` で根治。D015 resume run (= `1f50c53`) で本番効果実証 (= 親 + reply 2 本 + アフィリンク live で完走)。

### `/post-due` skill canonical 化

user の「`npm run publish:due` は Claude Code 側で実施できるよね？」発言起点で **`/post-due` skill 経由が canonical 経路**であることを明文化 (= 過去 chat session 内で publish を回す前提が手元実装と乖離していた)。launchd cron は手動 fallback 経路として残置、A 段階 4.4 の `metrics:sync` も同様に Claude Code 側で起動。

### D018 (= 段階A 4.1 残)

画像配置のみ完了。**status=draft で保留**:

- 画像: Unsplash Masha Rostovskaya「teapot on table」(= 実商品は楽天 `r10-h50phd` ピッチャーだが汎用ピッチャー画像で mood として使う、`visual_verified: true` + 「実商品でない mood 画像」注記)
- License: Unsplash License (attribution 不要・商用OK)、frontmatter `image_license: unsplash` で photographer/photo_url 機械可読
- **publish には非 aff ペア (= 同日それより早い時刻に非 affiliate 投稿) が必須** → 段階A 4.1 の operational carry-over

### frank_voice 既定の刷新 (= monetization reply chain は「1 行ティーザー + リンク」)

D015 resume 直前、user feedback「D15 のリプライは変えておいたよ。これくらいのあっさりテイストで、今後いいんだけどね」を受けて `dept/content/style/frank_voice.md` の monetization reply chain 既定を **「1 行ティーザー + リンク」**に明文化 (`820d3c9`)。= 過去の説明過多 reply を撤廃、Threads voice のあっさり感を構造化。

### 着地 (= 13 commit、PR #15 → MERGED → archive まで一直線)

```
f35df12 feat(publish): self-sustaining pipeline — pairing invariant + publish_stall fault + 2/day rotation
5e9a0c8 content(段階A): D019 empathy 短文を D015 の非 aff 先行ペアとして仕込む
a32a4e8 content(D015): reply chain を「1 行ティーザー + リンク」のあっさり版に
97f8c41 chore(content): scheduled publish D019 + D015 (×2 dup commit)
820d3c9 style(frank_voice): monetization reply chain を「1 行ティーザー + リンク」既定に明文化
32f68dc chore(metrics): sync metrics.csv (14 rows) — clear staleness fault
20bb1c5 docs(tasks): record 19:00 JST publish 結果 (D019 ✅ / D015 partial) + 4.4 done
da6f8b9 chore(metrics): published frontmatter に metrics 反映
c17ed8c content(D018): Unsplash 画像を添付 (mood image) + visual_verified
5bc2a29 fix(threads-api): reply-chain flaky の根治 — container-not-ready を backoff retry
1f50c53 publish(D015): resume 成功 → published (thread, 2 replies, アフィリンク live)
a7e58c9 Merge pull request #15 from tatoflam/publish-pipeline-self-sustaining
485e355 chore(openspec): archive publish-pipeline-self-sustaining + apply spec deltas
```

main 最新: `485e355` ← `a7e58c9` (PR #15 merge)。

### 最終状態

| 項目 | 状態 |
|---|---|
| 構造的詰みの撲滅 | ✅ published=0 ループ解消 (pairing 不変条件 + publish_stall fault + 2/日 rotation) |
| reply-chain flaky 根治 | ✅ `publishContainer` backoff retry、本番効果実証 |
| D015 | ✅ 完全公開 (アフィリンク live、2 replies) |
| D019 | ✅ 公開 (非 aff ペア、89 imp) |
| spec | ✅ 3 capability に ADDED 要件を反映、archive 済 |
| 健康診断 | ✅ `loop:check` 5/5 clean、metrics fresh |
| テスト | ✅ 新規 12 本 pass (709/711、残 2 は無関係 pre-existing `image_license_schema`) |
| 段階A 4.1 (D018) | 🟢 画像準備済、scheduling は人ゲート (非 aff ペア確保が要) |
| 段階C (5/日) | 別 change、milestone gated (= 安定 2/日が数週 BAN/警告ゼロで回ってから) |

### 構造的意義

- **「1 session で根本原因 → live 復旧 → 構造修正 → flaky 根治 → PR merge → archive」が 24h 跨ぎ 2.1MB に収まる**: 通常は週次サイクルで分解する工程を、deadlock の緊急性 + spec-driven OpenSpec workflow + reply-chain flaky 根治のオプション工事を同一 session に詰めた密度の高いラン。publish 自走化マイルストーンの「段階B」完了点
- **publish 自走化 2 段モデル**: 6/12 `publish-due-greedy-reorder` (= ordering の自動化) + 本 change `publish-pipeline-self-sustaining` (= pairing 不変条件 + buffer + stall fault) の 2 段で「published=0 詰みの構造的撲滅」が完了。3 段目 (上限 5/日 = 段階C) は別 change として独立化、milestone driven
- **`loop:check` invariant 増量の効用**: 5 番目の detector `publish_stall` が **沈黙の早期可視化** を担い、本セッションで実際に「96h since last post but 1 admissible draft」を fire して根本原因確定のトリガになった = invariant を増やすことで「自分の自動化で詰むケース」を後追いではなく検知時に気づける構造に
