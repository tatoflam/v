...
| 地盤改良 | ocean | 42 | 8b1f8490（2026-05-22） | 標準 3 部構成、8 案件ケーススタディ、Excel 4 ファイル + 事例フォルダから抽出。生成時は soffice 未導入で PDF QA 未実施 → LibreOffice 導入済のため再生成すれば QA 可能 |
| 自火報 | yellow | — | （未確認） | |
...
- 2026-05-19: 新規マニュアルは原則 **標準 3 部構成**（目的 / 業務マニュアル / ケーススタディ）とし CLAUDE.md に明文化（aa4f1029）— マニュアル間の構成一貫性を確保
- 2026-05-19: 対象者レベルの使い分け — 経験者向けは `buildChecklistSlide` 中心（抜け漏れ防止）、未経験者向けは `why` フィールド併用（教育の台本）。`why` と `note` は排他、両方必要な情報過多スライドは 2 枚に分割（aa4f1029）
- 2026-05-19: **Claude Code + pptxgenjs の現構成を維持**、Claude Design（Anthropic Labs の canvas UI ベース別製品）は CLI ツールではないため不採用、PPTX 系 MCP（office-pptx-mcp 等）は `pptxgenjs` と抽象度重複のため追加せず、Playwright MCP は同梱済で追加インストール不要（aa4f1029）— プログラマティック／再現可能な生成用途を優先、外部依存追加は最小化
- 2026-05-19: `assets/` 配下の**全ファイル**に目を通してから `generate.js` を書く原則（aa4f1029 で痛感）— 元資料の重要コンテンツの反映漏れ防止
- 2026-06-26: `buildEndSlide` は最初から **5 行以内**で設計する（d9595f4e）— 事実上の上限が 5 行で、6 行では下端からはみ出すため
...
### 2026-05-19: 地盤調査マニュアル再生成＋標準構成の確立（aa4f1029）

- 5-part（標準 3 部）構成を初実装（`manuals/地盤調査/generate.js`）、slide-builders 拡張、Claude Design 調査（→ 不採用判断、決定事項参照）
- 元資料を漏れなく反映する原則を痛感した回。`assets/` 配下からの反映内容:
...
- コンテンツが旧版比 **23% 増**（1.19 MB pptx）に到達。`_backup_20260422.pptx` を作って旧版を保全したうえで上書き再生成

### 2026-05-22: 地盤改良マニュアル新規生成（8b1f8490）
...
### 2026-06-26: 解体・家屋調査マニュアル新規生成＋LibreOffice 導入（d9595f4e）

ユーザ依頼「解体・土地家屋調査のマニュアルを作って！土地家屋調査については、どの範囲（隣接する家屋）を対象とするかについても記載すること」に対し、`manuals/解体・家屋調査/generate.js` を新規作成 → **31 スライド** (ocean) を生成。あわせて `brew install --cask libreoffice` で LibreOffice 26.2 を導入し PDF QA を実施可能に。
