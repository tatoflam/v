...
- **通知はメールのみ**（2026-06-07 の email-only 化以降。LINE 受信・ToDo 抽出・`line_group_id` は継続、廃止は日次レポートの LINE push のみ。`LineNotifier` はコード残置・`main.py` 非注入）
- **スケジューラは固定 crontab `*/15 * * * *`**（2026-06-12 のドリフト障害対応。照合窓 ±7 分は据え置き）
- **汎用リカバリ機能あり**: `daily_report_trigger?since=&until=&note=&label=`（任意時間窓を 1 通集約 + 注記バナー、冪等キー `label`）と `?date=&slot=` の過去日再送。Runbook: `docs/operations/runbook-missed-report.md`
- **PR #1** `fix/scheduler-drift-recovery` は 2026-06-24 マージ済（https://github.com/meguruit/ToDoBot/pull/1）、テスト **187 件** green
...
- 2026-06-07: **LINE プッシュを廃止しメール通知のみとする（email-only 化）** — 無料枠 200 通/月が 4 日で枯渇（通数 = 配信数 × 宛先人数 × 分割数、実績 6/1〜6/4 で 195 通）。ライトプラン ¥5,500/月 への引き上げより運用簡素化を選択。OpenSpec change `email-only-notifications` として実装・アーカイブ済
- 2026-06-07: **LINE 429 は即 fail してメール優先** — 429 リトライ（30s→2m→8m ≈10 分）が関数実行時間を食い潰しメール送信を巻き添えにする構造の解消。ステータス判定もメール単独に変更
- 2026-06-12: **Cloud Scheduler の相対スケジュール `every N minutes` は使わず固定 crontab 表記にする** — 相対スケジュールは発火位相が毎日ドリフトし、照合窓 ±7 分を外れるサイレント障害を起こす（6/12 実障害）
- 2026-06-12: **障害時の再送は「任意ウィンドウ 1 通集約 + 注記」を標準とする** — スロット単位の再実行でなく、取りこぼし期間全体を 1 通で補償する
...
```
curl -i -H "X-Job-Token: ${JOB_SHARED_SECRET}" \
  "https://asia-northeast1-todobot-dev.cloudfunctions.net/daily_report_trigger?profile=1課&since=<ISO>&until=<ISO>&note=<注記>&label=<冪等キー>"
```

- 任意の時間窓のメッセージを **1 通に集約**し注記バナー付きで送信。`?date=&slot=` で過去日スロット再送も可
- 手順書: `docs/operations/runbook-missed-report.md`（調査 + リカバリ + インシデント記録）。「そもそも起動しなかった」の検出は `alerting.md` の未送信ハートビート・アラート参照
- 実績: 6/12 ドリフト障害で 6/11 18:00 → 6/12 18:00 の 24h 分を集約再送（1課 46 件 / 2課 13 件 / team-b 0 件スキップ）

### `ALREADY_RAN` の手動解除（失敗ランの再実行）

**注意: 冪等ロックは失敗実行でも取得済みになる**ため、失敗ランの再送には Firestore のロック削除が必要（6/5 障害復旧で実証）。
...
- **2026-06-05〜07**（session 6c36b23b）: **通知全停止インシデント → email-only 化**。6/4 夜を最後に全グループの日次通知（メール・LINE とも）停止、webhook 受信は正常。原因 2 系統: ① LINE 月間プッシュ上限超過（無料 200 通/月を 6/4 に枯渇、以降 `429`。reply は無料のためチャット継続）② メール署名 API の接続プール汚染（`signBlob` の `service` オブジェクトをリトライで使い回し、SSL 切断 `UNEXPECTED_EOF` 後 4 回リトライ全滅。同一 tick で 1課成功・2課失敗が根拠）。修正: リトライ毎に signing transport 再生成 + LINE 429 即 fail 化（テスト 108 件）。復旧は冪等ロック手動削除 → 再送で 6/5 夜便配信（1課 44 件・2課 14 件）。**email-only 化を決定**し OpenSpec `email-only-notifications` 実装 → デプロイ → アーカイブ。commits `4be39bf` / `c794e57` / `242b41c` push。運用知識: LINE 通数 = 配信数 × 人数 × 分割数、プラン確認は manager.line.biz / gcloud・firebase CLI・ADC の認証は別管理で短時間失効（本セッション中 3 回再認証）
- **2026-06-12〜24**（session 41a0fb6a）: **6/12 通知未達（スケジューラドリフト）→ 恒久修正 + 汎用リカバリ + PR #1**。Cloud Scheduler `every 15 minutes` は「前回完了から N 分」の相対スケジュールで位相が毎日ドリフト、6/12 に tick が `:07:03`（スロットから 423 秒）まで後退し照合窓 ±7 分（420 秒）を 3 秒超過 → 06:00/18:00 とも 1 件も起動しないサイレント障害（失敗ログすら出ない。最終正常送信 6/11 18:00）。対応: ① 固定 crontab `*/15 * * * *` 化（窓は据え置き）② 汎用リカバリ `?since=&until=&note=&label=` + `?date=&slot=`（Runbook 参照）③ 6/11 18:00→6/12 18:00 の 24h 分を集約再送（当初 36h と誤認しかけたが 6/13 06:00 便は修正済スケジューラが配るため 24h が正解 — user 指摘が正しかった）④ `runbook-missed-report.md` 新設 + 未送信ハートビート・アラート（「起動しなかったこと」の検出が今回の盲点）。OpenSpec `missed-report-recovery` アーカイブ。**PR #1 を 6/24 user がマージ**、187 tests green
