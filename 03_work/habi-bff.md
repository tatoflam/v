...
## 2026-06-12 — プロダクト計画 × 実装状況 ステータスレポート HTML を PM/CTO 並列で起こす (session 5b553af9)

### 入口

「現状のプロダクト計画に対する実装状況と今後の実装プラン、現時点で見えている懸念点をまとめて、HTML フォーマットでレポート」依頼。最初は CTO 側だけのレポートを生成 → user フィードバックで **PM 側のチェックポイント到達状況も含めた PM ↔ CTO 並列構造** + **Mermaid 3 図 (システム全体図 / 4 step Processing Flow / PM ↔ CTO 監修フロー)** に拡張。`/tmp/habi-bff-status-report-20260612.html` ローカル生成のみで、配信なし (= 06_output 該当なし)。

### 状況スナップショット (= 2026-01 リリースまで残 7 ヶ月)

- M1–M2.1 完、M2.2 (Fast/Deep ルーティング) は v2.0 送り、M3 進行中、M4 以降未着手
- アクティブな OpenSpec change 3 本: `add-narrative-memory` **41/61**、`add-quality-always-on` **20/26**、`add-bff-policy-monitoring` **0/30**
- Narrative Memory: DynamoDB インフラ + リポジトリは着地、Processing Flow への配線は **habi-os M2.2 待ち** (= 現行 `SummarizeService.emotionalContext` がスペック違反になるため強引繋ぎ不可)
- Quality Always-On: CI 配線 + Runner + A/B 判定すべて完。**PM Seed 30 本オーサリングが残るクリティカルパス**

### PM 側の積み残し (= 本セッションで初可視化)

| 項目 | 状態 | 期限 / 影響 |
|---|---|---|
| Month 2 PM 監修 §1〜§10 | 10 項目すべて未確認 | 期限 **2026-05-14 から 29 日超過** (本ラン基準)、合計工数 2〜4 日見積 |
| M3 PM 設計タスク (意味粒度 / `memory_update_candidate` 6 サブタイプ / `relation_note` / `mid_summary`) | 7 項目すべて未着手 | Narrative Memory の意味設計の柱、CTO 側だけでは決められない |
| Quality Always-On Seed 30 本 | 0/30 | v1.0 リリースの単一クリティカルパス |
| Inquiry_20260508 思想前提共有 | 完了 (CTO 回答済み、CLAUDE.md 固定化済み) | — |

### 主な懸念 (再編)

- **R1 ★★★★★**: habi-os Deep Summarize 分離出力 PR が出るまで Narrative Memory が動かないインフラのまま
- **R2 ★★★★★**: PM Seed 30 本オーサリングが v1.0 リリースの単一クリティカルパス
- **R3 ★★★★☆**: M3→M12 を 7 ヶ月で押し切る耐性。R1 が滑ると M4 ごと連鎖遅延
- **P1〜P4** (PM 側): PM 監修オーバーラン / 意味粒度停滞 / Seed 30 本オーサリング / Step Debug 体感確認滞留

### 提言 (優先順)

1. habi-os に **Deep Summarize 分離出力 PR** を最優先で起票
2. `tests/quality/seeds/SEEDING_GUIDE.md` を先出しし、PM の Seed オーサリングを並行開始
3. BFF Policy Monitoring を **インフラだけ先に着地** (`MetricsBuilder` + `turn.completed` ログ)
4. PM 集中監修日の確保 + 意味粒度ワークショップ + Seed オーサリングのガイド化

### Mermaid 図の craft (= 3 round 反復)

レイアウト調整で **R-1 縦 (TB) → R-2 横 (LR) で圧縮 → R-3 メインを縦に戻し Narrative Memory を subgraph で横並び** に着地。BFF Guard verdict 4 種 + max_rewrite を 1 行に統合、各 BOX も `+ Boundary 事前 / 事後` などコンパクト記法に。点線エッジ (REWRITE / HOLD / Deep のみ) の Mermaid syntax 誤記 `-.|label|.->` → 正規 `-. "label" .->` への直しを 1 回踏んだ → 詳細は [[05_learn/mermaid-dotted-edge-label-syntax]]

### 副産物・学び

- **「PM/CTO 並列の Month 別表」が状況俯瞰の鍵**: 既存の HABI_DEVELOPMENT.md は CTO タスクが章立て主軸で、PM 側の積み残しがどこに刺さるかが直感的に取れなかった。1 行 = 1 Month × 列 = CTO / PM / 受入ブロッカー の表に再構築するだけで、待ち合わせ点が即可視化された
